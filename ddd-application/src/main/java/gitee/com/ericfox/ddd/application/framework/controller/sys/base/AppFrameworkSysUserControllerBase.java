...
public abstract class AppFrameworkSysUserControllerBase implements BaseController<SysUser, SysUserEntity, SysUserPageParam, SysUserDetailParam, SysUserDto> {
    ...
    public Mono<List<SysUserDto>> list(SysUserPageParam pageParam) {
        List<SysUserEntity> entityList = sysUserService.queryList(pageParam.toEntity(), pageParam.getPageSize());
        return Mono.just(Dto.fromEntityList(SysUserDto.class, entityList));
    }

    @Override
    @MessageMapping(SysUserDto.BUS_NAME + ".streamList")
    public Flux<SysUserDto> streamList(SysUserPageParam pageParam) {
        List<SysUserEntity> entityList = sysUserService.queryList(pageParam.toEntity(), pageParam.getPageSize());
        return Flux.fromStream(Stream.of(ArrayUtil.toArray(Dto.fromEntityList(SysUserDto.class, entityList), SysUserDto.class)));
    }
    ...
    public Mono<SysUserDto> create(@RequestBody SysUserDetailParam detailParam) {
        ...
        entity = sysUserService.insert(entity);
